...
void uart_Rx_interrupt_init(void)
{
    ...
    RCC->APB2ENR |= RCC_APB2ENR_AFIOEN; // <-- REQUIRED FOR AFIO / UART
    ...
    // calculate mantissa
    uint16_t usartdiv = SYSTEM_CLOCK / (16 * BAUD_RATE);
    uint16_t mantissa = usartdiv;
    uint16_t fraction = ((SYSTEM_CLOCK % (16 * BAUD_RATE)) * 16) / (16 * BAUD_RATE);
    USART1->BRR = (mantissa << 4) | (fraction & 0xF);

    // use this TO SET for 8MHz and 9600 baud instead of calculating if there is any issue
    //  USART1->BRR = 0x341;
    ...
    volatile uint16_t temp = USART1->SR; // you must clear any pending flags
    temp = USART1->DR;                   // flush old data
